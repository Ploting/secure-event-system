...
const getEventId = async (req, res) => {
  try {
    ...
    const [selectedRow] = await db.query("Select * from events where id = ?", [
      eventId,
    ]);
...
const updateEvent = async (req, res) => {
  ...
  try {
   ...
   WHERE id = ? AND is_deleted = FALSE`,
      [title, description, location, event_date, eventId],
    );
...
const deleteEvent = async (req, res) => {
  ...
  try {
    ...
    const [deleteEvent] = await db.query(
      "UPDATE events SET is_deleted = ? WHERE id = ?",
      [eventId, true],
    );
